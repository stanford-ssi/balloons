...
void setup() {
  ...
  MS_5803.reset();
  if(!MS_5803.begin()){Serial.println("Could not find a valid MS5803 sensor, check wiring!");} 
...
static void readData(void){
  ...
  PRESS_MS5803 = MS_5803.getPressure(ADC_4096);// * 10;
  TEMP_MS5803  = MS_5803.getTemperature(CELSIUS, ADC_512);
  TIMER        = getTime();
  BLINK = !BLINK;
}
