...
/* ****************  TEENSY PIN OUTS  ****************  */
uint8_t  SD_CD             =   2;
uint8_t  ROCKBLOCK_SLEEP   =   9; //Current version
uint8_t SD_CS              =  21; //Current version
uint8_t VMON               =  14;
uint8_t IMON               =  30; //Current version
uint8_t BMP280_CS          =  20; //Current version
uint8_t BMP280_CS_2        =  15; //Current version
uint8_t THERMOCPL_CS       =  22; //Current version
uint8_t HEATER = 43;
uint8_t CUTDOWN = 44;
//Add in MPL
//GPS is on Serial1 (pins 0 and 1)
//Pin 2 is GPS reset
//Pin 26 is GPS enable

//Radio enable is 14
//RB is on Serial3 (pins 7 and 8)
...
uint8_t HEARTBEAT          =   3;
uint8_t I_GOOD             =   5; //Current version
uint8_t T_GOOD             =   7; //Current version
uint8_t P_GOOD             =   6; //Current version
uint8_t RB_GOOD            =   1;
uint8_t GPS_GOOD           =   2; //Current version
uint8_t BAT_GOOD           =   4; //Current version
uint8_t CAN_GOOD           =   0; //Current version
...
/*
   ...
   usage: initSensors();
   ---------------------------------
   This function initializes the BMP280, and MCP23017.
*/
...
static void initSensors(void){
  ...
  Teensy3Clock.set(1357041600); // set the RTC
  setTime(1357041600);
  if(!bme.begin()) {
...
static String getTime(void){
  String timer = "";
  timer+= hour();
  timer+= printDigits(minute());
  timer+= printDigits(second());
  timer+= '/';
  timer+= year();
  timer+= '/';
  timer+= month();
