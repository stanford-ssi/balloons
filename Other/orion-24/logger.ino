...
// heater digital output
int HEATER_PIN = 4;
const int MIN_TEMP = 10; // in Celcius
const int xPin = 0;
const int yPin = 1;
const int zPin = 2;
const int chipSelect = 10; // SD Pin
...
void loop() {
  ...
  if (millis() - logTime > 2000) {
    ...
    //    Serial.println(Temp);
    if (Temp < MIN_TEMP) {
      digitalWrite(HEATER_PIN, HIGH); //make sure we are writing at least 2.5V to keep the transistor on
    } else {
      digitalWrite(HEATER_PIN, 0); // 5 is MOSFET Pin
    }
